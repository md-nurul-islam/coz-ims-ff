...
class QualitiesController extends Controller {
    ...
    public function actionView($id) {
        $this->pageTitle = Yii::app()->name . ' - Quality Details';
        $this->pageHeader = 'Quality Details';
    ...
    public function actionCreate() {
        ...
        $this->pageHeader = 'Add Quality';
        $this->pageTitle = Yii::app()->name . ' - Quality Size';

        if (isset($_POST['Grade'])) {
            $model->attributes = $_POST['Grade'];
            $model->name = ucfirst(trim($model->name));
            $model->code = strtoupper(Settings::getToken(4));
            
            if ($model->save()) {
                Yii::app()->user->setFlash('success', 'Quality successfully added.');
    ...
    public function actionUpdate($id) {
        ...
        $this->pageTitle = Yii::app()->name . ' - Update Quality';
        $this->pageHeader = 'Update Quality';
        ...
        if (isset($_POST['Grade'])) {
            ...
            if ($model->save()) {
                Yii::app()->user->setFlash('success', 'Quality successfully updated.');
    ...
    public function actionIndex() {
        $this->pageTitle = Yii::app()->name . ' - Quality List';
        $this->pageHeader = 'Quality List';
        $this->render('index');
    }
...
//        var_dump($result['rows']);exit;
    ...
    public function actionGetStatusComboData() {
        echo CJSON::encode(Grade::model()->statusComboData());
    }
